...
def doc_to_visual(doc: dict, lmms_eval_specific_kwargs: Optional[dict] = None) -> List[str]:
...
def doc_to_text(doc: dict, lmms_eval_specific_kwargs: Optional[dict] = None) -> str:
...
def _extract_letter(text: str) -> str:
    ...
    # Pattern: "(A)" or just single letter
    match = re.match(r"^\(?([A-Z])\)?$", text.strip(), re.IGNORECASE)
    if match:
        return match.group(1).upper()

    # Find any standalone letter A-Z
...
def process_results(doc: dict, results: list) -> Dict[str, dict]:
    ...
    if len(answer) == 1 and answer.upper() in string.ascii_uppercase:
        # Answer is already a letter (ssv2, kth)
        gt_letter = answer.upper()
    else:
        # Answer is text — find matching candidate
        gt_letter = None
        ...
        if gt_letter is None:
            eval_logger.warning(
                f"Answer '{answer}' not found in candidates: {candidates}"
            )
            gt_letter = "?"
